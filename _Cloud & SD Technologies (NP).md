...
~~~
!@cmd
route add 10.0.0.0 mask 255.0.0.0 10.42.1.4
route add 200.0.0.0 mask 255.255.255.0 10.42.1.4
~~~
...
conf t
 ...
 int gi0/1
  ...
  exit
 ip routing
 ip route 0.0.0.0 0.0.0.0 10.42.42.1 120
 enable secret pass
 line vty 0 14
  password pass
  transport input all
...
conf t
 int fa0/0
  ...
  exit
 ip routing
 ip route 0.0.0.0 0.0.0.0 10.42.100.4 120
 enable secret pass
 line vty 0 14
  password pass
  transport input all
...
conf t
 int gi 0/0/0
  ...
  exit
 ip routing
 ip route 10.42.0.0 255.255.0.0 10.42.42.4 120
 enable secret pass
 line vty 0 14
  password pass
  transport input all
...
| Description                      | Command                                  |
| ---                              | ---                                      |
| a non-default hostname           | hostname coreBaba42                  |
...
ip route add 10.0.0.0/8 via 10.42.1.4 dev ens256
ip route add 200.0.0.0/24 via 10.42.1.4 dev ens256
...
~~~
!@NetOps
ansible-playbook -i rivan_mkt.ini playbooks/deploy_42.yml --skip-tags ivrs
~~~
